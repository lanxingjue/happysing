...
```meriad
graph TD
    A[用户界面 GUI] --> B(核心逻辑 Controller)
    B --> C[音频处理 Audio Processor]
    B --> D[学习管理 Learning Manager]
    B --> E[游戏化/反馈系统 Gamification/Feedback]
    B --> F(数据层 Data Layer)
    C --> B
    F --> B
    F --> D
    ...
    subgraph GUI
        A1(主窗口 Main Window)
        A2(歌曲选择界面 Song Select)
        A3(学习界面 Learning Screen)
        A4(反馈动画/弹窗 Feedback Popups/Animations)
        A5(游戏元素显示 Game UI)
        A --> A1 & A2 & A3 & A4 & A5
    end
...
**当前版本：** Step 12 - 优化反馈与UI/UX（动画、背景与视觉指示器）

**功能说明：**
*   程序启动时从 `data/songs.json` 加载歌曲信息，从 `data/user_progress.json` 加载用户进度。
*   歌曲选择界面根据用户进度显示歌曲状态。
*   点击歌曲按钮切换到学习界面。学习界面顶部显示当前用户的总星星数。
*   学习界面 **新增背景图片**（如果歌曲数据指定）。
*   中间显示当前乐句歌词，样式 **已优化**。
*   “听一听”按钮播放当前乐句音频并 **高亮歌词**。
*   “我来唱”按钮连接到录音功能。
*   录音停止后，程序分析录制音频的音量、音高、节奏。
*   根据分析结果，在反馈区显示一个 **角色动画（GIF）或图片** 和积极的文字反馈。
*   **新增:** 在反馈区下方显示简单的 **视觉指示器**（例如图标 + 符号）来展示音量、音高和节奏的分析结果。
*   获得的星星会累加到用户的总星星数中，并自动保存。
*   “下一句”按钮切换到下一乐句。
*   完成歌曲后，触发闯关完成和歌曲解锁逻辑。
